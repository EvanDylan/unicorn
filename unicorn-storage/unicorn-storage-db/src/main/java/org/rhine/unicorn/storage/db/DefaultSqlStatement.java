...
public class DefaultSqlStatement {
    ...
    private static final String INSERT_SQL = String.format("INSERT INTO %s (`flag`, `service_name`, `name`, `key`, `class_name` `response`, `created_time`, `expired_time`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", TABLE_NAME);
    private static final String UPDATE_SQL = String.format("UPDATE %s SET `expired_time` = ? WHERE `id` = ? ;", TABLE_NAME);
    private static final String QUERY_SQL = String.format("SELECT * FROM %s WHERE `service_name` = ? and `name` = ? and `key` = ? ;", TABLE_NAME);
